...
export class CustomCollectionController {
    constructor(private readonly customCollectionService: CustomCollectionService) { }
    ...
    @Post()
    async createCustomCollection(@Body() customCollectionPostDto: CutomCollectionPostDto, @Query() queryParams: ShopParams): Promise<any> {
        return await this.customCollectionService.createCustomCollection(customCollectionPostDto, queryParams);
    }
}
